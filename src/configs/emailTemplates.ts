import { sendEmail, EmailOptions } from './emails';
import env from './env';
...
/**
 * Welcome email – sent after successful verification + onboarding
 */
export const sendWelcomeEmail = async (
  to: string,
  data: {
    name: string; // User's name
    organizationName: string; // Name of the first org they created or joined
    role: 'CA' | 'OWNER' | 'STAFF'; // Their role in the org
    dashboardLink?: string; // Optional deep link to dashboard (e.g. with active org pre-selected)
  }
): Promise<boolean> => {
  const preheader = `Welcome to Finager India – Your account is ready!`;
  ...
  <head>
	...
	<title>Welcome to Finager India</title>
		  ...
		  <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width:600px; background-color:#ffffff; border-radius:12px; overflow:hidden; box-shadow:0 4px 20px rgba(0,0,0,0.08);">
			...
			<tr>
			  <td style="padding:32px 40px 20px; background: linear-gradient(135deg, #6366f1 0%, #4f46e5 100%); text-align:center;">
				...
				<p style="margin:12px 0 0; font-size:18px; color:#ffffff; opacity:0.9;">
				  Your Finager India account is now active
				</p>
			  </td>
			</tr>
			...
			<tr>
			  <td style="padding:40px 40px 32px; font-size:16px; line-height:1.6; color:#1f2937;">
				...
				<p style="margin:0 0 24px;">
				  Congratulations! Your email is verified, and your account with <strong>Finager India</strong> is fully set up.
				</p>
  
				<p style="margin:0 0 24px;">
				  ${
            data.organizationName
              ? `You are now part of <strong>${data.organizationName}</strong> as a <strong>${data.role === 'CA' ? 'Chartered Accountant' : data.role === 'OWNER' ? 'Owner' : 'Staff Member'}</strong>.`
              : `You're ready to create or join organizations and start managing finances smarter.`
          }
				</p>
  
				<p style="margin:0 0 32px;">
				  With Finager India, you and your team can now:
				</p>
  
				<ul style="margin:0 0 32px; padding-left:20px; color:#4b5563; line-height:1.8;">
				  <li>Record billing, purchases, and payments in real time</li>
				  <li>Track inventory and customers/vendors effortlessly</li>
				  <li>Generate accurate journals and reports automatically</li>
				  <li>Reconcile GST with GSTR-2B and prepare GSTR-3B summaries</li>
				  <li>Collaborate securely with your CA or clients — no more file chasing</li>
				</ul>
				...
				<table role="presentation" border="0" cellpadding="0" cellspacing="0" style="margin:0 auto 32px;">
				  <tr>
					<td style="border-radius:8px; background-color:#4f46e5;">
					  <a href="${data.dashboardLink || 'https://app.finagerindia.com/dashboard'}" 
						 target="_blank" 
				...
				<p style="margin:0 0 24px; font-size:15px; color:#4b5563;">
				  If you're a CA, start inviting clients. If you're a business owner, begin recording transactions — your CA will see everything in real time.
				</p>
  
				<p style="margin:0; font-size:14px; color:#6b7280; text-align:center;">
				  Questions? Reach us anytime at <a href="mailto:support@finagerindia.com">support@finagerindia.com</a>
				</p>
			  </td>
			</tr>
  ...
  const text = `
  Welcome to Finager India!
  
  Hello ${data.name},
  
  Congratulations! Your email is verified and your Finager India account is now fully active.
  
  ${
    data.organizationName
      ? `You are now part of ${data.organizationName} as a ${data.role === 'CA' ? 'Chartered Accountant' : data.role === 'OWNER' ? 'Owner' : 'Staff Member'}.`
      : `You're ready to create or join organizations and take control of your finances.`
  }
  
  With Finager India, you can now:
  - Record billing, purchases, and payments in real time
  - Track inventory and contacts effortlessly
  - Generate accurate accounting entries and reports automatically
  - Reconcile GST safely and prepare returns
  
  Get started here: ${data.dashboardLink || 'https://app.finagerindia.com/dashboard'}
  
  If you have any questions, reach out to us at support@finagerindia.com.
  ...
  return sendEmail({
    to,
    subject: `Welcome to Finager India – Your Account is Ready!`,
    html,
    text,
  });
};
...
/**
 * Password Reset Email – Finager India branded
 */
...
/**
 * Email verification template – Finager India branded
 */
			...
			<tr>
			  <td style="padding:40px 40px 32px; font-size:16px; line-height:1.6; color:#1f2937;">
				...
				<table role="presentation" border="0" cellpadding="0" cellspacing="0" style="margin:0 auto 32px;">
				  <tr>
					<td style="border-radius:8px; background-color:#4f46e5;">
					  <a href="${env.FRONTEND_URL}/auth/emailVerified?token=${data.token}" 
						 target="_blank" 
						 style="display:inline-block; padding:16px 56px; font-size:18px; font-weight:600; color:#ffffff; text-decoration:none;">
						Verify My Email →
					  </a>
				...
				<p style="margin:0 0 24px; font-size:14px; color:#6b7280; text-align:center;">
				  Or copy and paste this link into your browser:<br>
				  <a href="${env.FRONTEND_URL}/auth/emailVerified?token=${data.token}" style="color:#4f46e5; word-break:break-all;">${env.FRONTEND_URL}/auth/emailVerified?token=${data.token}</a>
				</p>
          ...
          data.actionLink && data.actionText
            ? `
					<div style="text-align: center; margin: 30px 0;">
						<a href="${data.actionLink}" style="background: #667eea; color: white; padding: 12px 30px; text-decoration: none; border-radius: 5px; display: inline-block;">${data.actionText}</a>
					</div>
				`
            : ''
        }
